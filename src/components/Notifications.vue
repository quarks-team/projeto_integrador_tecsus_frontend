    ...
    <div class="notifications">
        ...
        <h2 class="title">Notifications
            <div class="notification-filter">
                <b @click.prevent="water = !water" :style="water === true ? { 'background-color' : '#004AEB', 'color' : 'white','white-space': 'nowrap','overflow': 'hidden','text-overflow': 'ellipsis' } : { 'background-color' : 'transparent', 'color' : '#004AEB','white-space': 'nowrap','overflow': 'hidden','text-overflow': 'ellipsis' }">water</b>
                <b @click.prevent="energy = !energy" :style="energy === true ? { 'background-color' : '#004AEB', 'color' : 'white', 'white-space': 'nowrap','overflow': 'hidden','text-overflow': 'ellipsis'} : { 'background-color' : 'transparent', 'color' : '#004AEB','white-space': 'nowrap','overflow': 'hidden','text-overflow': 'ellipsis' }">energy</b>
                <b @click.prevent="sink = !sink" :style="sink === true ? { 'background-color' : '#004AEB', 'color' : 'white' , 'white-space': 'nowrap','overflow': 'hidden','text-overflow': 'ellipsis' } : { 'background-color' : 'transparent', 'color' : '#004AEB','white-space': 'nowrap','overflow': 'hidden','text-overflow': 'ellipsis' }">New Sb</b>
            </div>
        </h2>    
    
        <div class="card-notification" v-for="n in notifications" :key="n.id">
            <h4 v-if="(n.operation === 'Changed Sb status' || n.operation === 'Unlocking items by the new Sb') && n.item_status === 'INCORPORATED'" @click.prevent="divClickToWater(n.chassis_associate)">
            ...
            <h5 v-if="(n.operation === 'Changed Sb status' || n.operation === 'Unlocking items by the new Sb') && n.item_status === 'APPLICABLE'" @click.prevent="divClickToWater(n.chassis_associate)">
                energy of "{{ n.item }}" unlocking in chassis {{ n.chassis_associate }} 
            </h5>

            <h4 v-if="n.operation === 'Changed Sb status'" @click.prevent="divClickToEnergy(n.sb_name, n.sb_part)">
                By change in {{ n.sb_name }} {{ n.sb_part }} for <b>{{ n.sb_status_change }}</b>
                <b :style="n.item_status === 'INCORPORATED' ? { 'background-color': '#548644', 'color' : 'white' } : { 'background-color': '#7CF0BD', 'color' : 'white' }">{{ n.item_status }}</b>
                <button class="button-delete" @click.prevent="deleteNotification(n.id)"><i class="fa-solid fa-trash"></i></button>
            </h4>
            <h5 v-if="n.operation === 'Unlocking items by the new Sb'" @click.prevent="divClickToEnergy(n.sb_name, n.sb_part)">
                By creation of {{ n.sb_name }} {{ n.sb_part }} with status <p>{{ n.sb_status_change }}</p>
            </h5>
            
            <h4 v-if="n.operation === 'Creation of new Sb'" @click.prevent="divClickToSink(n.chassis_associate)">
                Creation of new Service Bulletin in chassis {{ n.chassis_associate }}
                <button class="button-delete" @click.prevent="deleteNotification(n.id)"><i class="fa-solid fa-trash"></i></button>
            </h4>
            <h5 v-if="n.operation === 'Creation of new Sb'" @click.prevent="divClickToSink(n.sb_name, n.sb_part)">
...
export default {
    ...
    methods: {
        ...
        filterWater() {
            if (!this.water) {
                this.filteredWater = this.notifications.filter(item => String(item.item_status) === 'INCORPORATED');
        ...
        filterEnergy() {
            if (!this.energy) {
                this.filteredEnergy = this.notifications.filter(item => String(item.item_status) === 'APPLICABLE');
        ...
        filterSink() {
            if (!this.sink) {
                this.filteredSink = this.notifications.filter(item => String(item.item_status) !== 'INCORPORATED' && String(item.item_status) !== 'APPLICABLE');
